...
def PaxosNode(node_id, value, numNodes, prob, numRounds, barrier):
    ...
    maxVotedVal = -1  # Proposer & Acceptor
    ...
    for r in range(numRounds):
        ...
        if is_proposer:
            ...
            if "PREPARE" in message_received_body:
                ...
                if maxVotedRound == -1:
                    ...
                    sendRegular(
                        body=f"PROMISE {r} accepted {maxVotedRound}, {maxVotedVal} ",
                        sender_id=node_id,
                        target_id=message_received_from,
                        prob=prob,
                        push_socket=push_sockets_dict[message_received_from],
        ...
        if is_proposer:
            ...
            if will_propose:
                ...
                print(f"PROPOSER do Round {r} cancelou o round por falta de quorum, valor se mantém: {proposeVal} ")
                broadcast(
                    body="ROUNDCHANGE",
                    sender_id=node_id,
                    numNodes=numNodes,
        ...
        if is_proposer:
            ...
            if will_propose:
                ...
                if "ROUNDCHANGE" not in message_received_body:
                    ...
                    for _ in range(numNodes - 1):
                        ...
                        print(
                            f"PROPOSER OF {node_id} SEND IN ACCEPT PHASE: {message_received_body}"
                        )
            ...
            if "ACCEPT_REQUEST" in message_received_body:
                ...
                maxVotedRound = r
                maxVotedVal = int(message_received_body.split(" ")[2])
